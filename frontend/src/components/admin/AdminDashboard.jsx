...
import { 
  ...
  Download, Search, Filter, Calendar, MapPin, Clock, Mail, Phone,
  AlertCircle, CheckCircle, XCircle, Ban, Trash2, Edit, Send,
  BarChart3, TrendingUp, Activity, Shield, Lock, Unlock, UserPlus,
  RefreshCw, Bell, Menu, ChevronLeft, ChevronRight, MoreVertical
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const [stats, setStats] = useState({
    ...
    totalStaff: 0, activeStaff: 0, pendingPayments: 0, totalRevenue: 3250000
  });
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Failed to load data. Please clock in again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleValidateStudent = async (studentId, status) => {
    await fetch('/api/admin/students/validate', {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        Authorization: `Bearer ${sessionStorage.getItem('adminToken')}`
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gray-900 text-white transition-all duration-300`}>
        ...
        <nav className="mt-8">
          ...
          <button onClick={logout} className="w-full flex items-center gap-4 px-6 py-4 hover:bg-red-900 transition mt-auto">
      ...
      <div className="flex-1 p-8">
        {loading ? (
          ...
          <>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    ...
                    { label: 'Pending Validation', value: stats.pendingStudents, icon: AlertCircle, color: 'yellow' },
                    { label: 'Active Staff', value: stats.activeStaff, icon: UserCheck, color: 'green' },
                    { label: 'Revenue', value: `₦${stats.totalRevenue.toLocaleString()}`, icon: TrendingUp, color: 'purple' },
                  ].map((stat, i) => (
            ...
            {currentView === 'students' && (
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-8">All Students</h2>
                    ...
                    <tbody>
                      {students.map(student => (
                        <tr key={student.student_id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4">
                            {student.status === 'pending' && (
                              <button onClick={() => handleValidateStudent(student.student_id, 'active')} className="text-green-600 hover:text-green-800 mr-3">
